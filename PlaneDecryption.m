function DecImage = PlaneDecryption(DecDNAImage)
   % 获取图像大小
    [rows, cols] = size(DecDNAImage);
    
    % 对图像进行位平面分解
    I_bitPlanes= zeros(rows, cols, 8);
    I_bitPlanes(:,:,1)= bitget(DecDNAImage, 1); % 位平面分解，得到大小为 rows x cols x 8 的位平面矩阵
    I_bitPlanes(:,:,2)= bitget(DecDNAImage, 2);
    I_bitPlanes(:,:,3)= bitget(DecDNAImage, 3);
    I_bitPlanes(:,:,4)= bitget(DecDNAImage, 4);
    I_bitPlanes(:,:,5)= bitget(DecDNAImage, 5);
    I_bitPlanes(:,:,6)= bitget(DecDNAImage, 6);
    I_bitPlanes(:,:,7)= bitget(DecDNAImage, 7);
    I_bitPlanes(:,:,8)= bitget(DecDNAImage, 8);
    ...
    DecImage = uint8(0);
    for i = 1:8
        DecImage = bitor(DecImage, uint8(I_bitPlanes(:,:,i)) * 2^(i-1));
    end
end
